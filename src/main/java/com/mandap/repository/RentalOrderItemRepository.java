...
public interface RentalOrderItemRepository extends JpaRepository<RentalOrderItem, Long> {
    ...
    @Query("SELECT roi FROM RentalOrderItem roi WHERE roi.inventoryItem.id = :inventoryItemId AND roi.dispatchedQty > roi.returnedQty")
    List<RentalOrderItem> findOutstandingByInventoryItem(Long inventoryItemId);
}
