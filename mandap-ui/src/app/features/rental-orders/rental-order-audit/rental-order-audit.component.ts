...
import { RentalOrderService } from '@core/services';
import { DateFormatPipe } from '@shared';

@Component({
    selector: 'app-rental-order-audit',
    standalone: true,
    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-3xl font-bold text-[var(--color-text-primary)]">Rental Order Audit Log</h1>
            <p class="text-[var(--color-text-secondary)] mt-1">
              History for Order: <span class="text-teal-400 font-mono font-semibold">{{ orderNumber() }}</span>
            </p>
          </div>
        </div>
      </div>
          ...
          <div class="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-700 before:to-transparent">
            ...
            @if (auditHistory().length === 0) {
              <div class="text-center py-20 bg-[var(--color-bg-card)] rounded-3xl border border-dashed border-slate-700">
                ...
                <p class="text-slate-400">No audit logs available for this order.</p>
              </div>
            }
          </div>
        </div>
...
export class RentalOrderAuditComponent implements OnInit {
    ...
    loadAudit(id: number): void {
        // Also fetch order info for display
        this.rentalOrderService.getById(id).subscribe(order => this.orderNumber.set(order.orderNumber || 'Order'));

        this.rentalOrderService.getAuditHistory(id).subscribe({
            next: (data) => {
