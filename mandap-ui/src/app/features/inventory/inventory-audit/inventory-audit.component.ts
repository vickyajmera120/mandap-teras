    ...
    <div class="space-y-6">
      ...
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-3xl font-bold text-[var(--color-text-primary)]">Inventory Audit Log</h1>
            <p class="text-[var(--color-text-secondary)] mt-1">
              History for: <span class="text-teal-400 font-semibold">{{ itemName() }}</span>
            </p>
          </div>
        </div>
      </div>
          ...
          <div class="relative space-y-8 before:absolute before:inset-0 before:ml-5 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-slate-700 before:to-transparent">
            ...
            @if (auditHistory().length === 0) {
              <div class="text-center py-20 bg-[var(--color-bg-card)] rounded-3xl border border-dashed border-slate-700">
                ...
                <p class="text-slate-400">No audit logs available for this item.</p>
              </div>
            }
          </div>
        </div>
...
export class InventoryAuditComponent implements OnInit {
    ...
    loadAudit(id: number): void {
        // Also fetch item info for name display
        this.inventoryService.getById(id).subscribe(item => this.itemName.set(item.nameGujarati));

        this.inventoryService.getAuditHistory(id).subscribe({
            next: (data) => {
